...
function createBroadwayLights() {
  const containers = document.querySelectorAll(".project-template_stats_card");
  ...
  containers.forEach((container) => {
    ...
    // Get container dimensions using the content box rather than the rotated bounding box
    const width = container.offsetWidth;
    const height = container.offsetHeight;

    // Determine if this is the rotated card
    const isRotatedCard = container.classList.contains(
      "project-template_stats_card"
    );

    // Configuration - adjust the number of lights based on if it's the rotated card
    const horizontalCircles = isRotatedCard ? 8 : 12; // fewer circles on top/bottom for rotated card
    const verticalCircles = isRotatedCard ? 10 : 14; // fewer circles on left/right for rotated card
    ...
    const createCircle = (x, y, index) => {
      ...
      // Set the width and height directly in rem
...
function getResponsiveSizes() {
  ...
  if (viewportWidth >= 992) {
    ...
    // Tablet
    return {
      circleSize: 0.8, // Smaller circles on tablet
      inset: 0.8,
    };
  } else if (viewportWidth >= 480) {
    // Mobile Landscape
    return {
      circleSize: 0.7, // Even smaller circles on mobile landscape
      inset: 0.8,
    };
  } else {
    // Mobile Portrait
    return {
      circleSize: 0.5, // Smallest circles on mobile portrait
      inset: 0.7,
    };
  }
}
